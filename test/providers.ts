...
describe('providers', function() {
  ...
  describe('DatabaseAbiProvider', async() => {
    ...
    it('detects proxy contract', async() => {
      ...
      const abiProxyByConstructor = await p.getBare(contractAddressProxyByConstructor, blockNumber)
      ...
      const abiProxyByEvent = await p.getBare(contractAddressProxyByEvent, blockNumber)
      log(abiProxyByEvent)
      ...
      const abiProxyByReadFunction = await p.getBare(contractAddressProxyByReadFunction, blockNumber)
      ...
      const abiRegular = await p.getBare(contractAddressRegular, blockNumber)
      log(abiRegular)
    ...
    it('gets abi for regular contract', async() => {
      ...
      const abiBare = await p.getBare(contractAddress, blockNumber)
      log(abiBare)
    ...
    it('gets abi for proxy contract', async() => {
      ...
      const abiProxy = await p.getBare(contractAddress, blockNumber)
      log(abiProxy)
    ...
    it('organizeEvent for proxy contract', async () => {
      ...
      const transactionCallOrganizer = new TransactionCallOrganizer(new DatabaseAbiProvider(defaultProvider, ds) /*new OnlineAbiProvider(defaultProvider)*/)
    ...
    xit('organizeBlock problem blocks', async function () {
      const blocks = [177233, /*161358, 171140, 181079, 120071, 120154, 120263, 110349, 110365, 110366, 110374, 110379, 110381, 110386, 110396, 110397, 110400, 130057, 130076*/]

      const blockOrganizer = new BlockOrganizer(new DatabaseAbiProvider(defaultProvider, ds) /*new OnlineAbiProvider(defaultProvider)*/)
