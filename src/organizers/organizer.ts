...
import {FunctionInput, OrganizedFunction, OrganizedEvent, StarknetArgument} from "../types/organize-starknet"
...
import {InvokeFunctionTransaction, DeployTransaction, Transaction, TransactionReceipt} from "../types/raw-starknet"
...
export class Organizer {
  ...
  async organizeEvents(transaction: Transaction | TransactionReceipt, blockNumber: number, blockHash?: string) {
    ...
    for (const event of transaction.events) {
      ...
      try {
        ...
        const m = `caught while organizing events for tx ${transaction} block ${blockNumber} err=${err}`
        console.error(m, err)
        throw err
      }
    }
