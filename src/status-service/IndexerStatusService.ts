...
import {StarknetBlockInfo} from '../starknet/data'
...
const debug = Debug('starknet-indexer:status-server')
...
export class IndexerStatusService implements IStatusService {
  ...
  async onBlockComplete(payload: StarknetBlockInfo): Promise<void> {
  ...
  async onNewMessage(channel: string, message: string): Promise<void> {
    if (channel === BLOCK_COMPLETE_CHANNEL) {
      const payload = JSON.parse(message) as StarknetBlockInfo
  ...
  async updateLastEvents(payload: StarknetBlockInfo): Promise<void> {
    //TODO use events or transactions in StarknetBlockInfo
    /*if (!payload.events) {
  ...
  async updateBlockCache(payload: StarknetBlockInfo): Promise<void> {
    await this.redisClient.set(
